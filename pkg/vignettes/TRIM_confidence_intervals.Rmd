---
title:  "R-TRIM confidence intervals"
author: "Patrick Bogaart"
institute: "Statistics Netherlands"
...
In this document, we well present a method to converted TRIM standard errors to, say, 95% coinfidence intervals, by approximating the underlying (discrete) Poison or negative binomial distributions as a (continuous) gamma distribution.
...
Multipliers relating standard errors with confidence intervals bounds can be computed using the same approach as demonstrated above for the Poisson distribution. Here is an example for $\sigma^2 = 1 \ldots 10$
...
In this case, the effect of overdispersion is very limited. For an example where overdispersion is large, see the vignettes [taming_overdispersion.html](Taming overdispersion) and [rtrim_2_extensions.html](rtrim 2.0 extensions)
