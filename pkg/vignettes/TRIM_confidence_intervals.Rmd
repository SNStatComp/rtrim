---
title:  "TRIM confidence intervals"
...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{trim-confidence-intervals}
...
Traditionally, TRIM estimates model parameters, along with their uncertainty expressed as an standard error (i.e., square root of their variance).

To assist comparison with other trend analysis methods, it might be helpfull of the TRIM output uncertainties can be presented as confidence intervals. One example is UIndex, but there might be more.

In this document, we well present a method to converted TRIM standard errors to, say, 95% coinfidence intervals, by approximating the underlying (concrete) Poison or negative binomial distributions as a (continuous) gamma distribution.
...
plot(NULL,NULL, type='n', xlim=range(x), ylim=range(y), xlab=NA, ylab="Probability density")

# background: confidence interval
xci <- seq(lo, hi, len=100)
yci <- dnorm(xci, mean=mu, sd=sd)
xx <- c(xci, rev(xci))
yy <- c(0*yci, rev(yci))
polygon(xx,yy,col=gray(0.9), border=NA)

# Foreground: complete distribution
...
Two observations are relevant here. First, the confidence interval of the distribution is found using the so-called quantile function, which is the inverse of the cumulative distribution. Second, the multiplicative factor 1.96 relating confidence interval and standard deviation is constant:
```{r}
mu  <- runif(5, -20, 20) # 5 random mu's
sd  <- runif(5, 1, 10)   # 5 random s.d.'s
hi  <- qnorm(0.975, mean=mu, sd=sd)
mul <- (hi-mu)/sd # compute the multiplier
data.frame(mu=mu, sd=sd, mul=mul)
```
...
sd <- 1.0
alpha <- 0.05
xcdf <- seq(mu-3*sd, mu+3*sd, len=100)
ycdf <- pnorm(xcdf, mean=mu, sd=sd)
plot(NULL,NULL, xlim=range(xcdf), ylim=c(0,1), ylab="c.d.f.")
# connect mu with median
x0 <- min(xcdf)
for (p in c(alpha/2, 0.5, 1-alpha/2)) {
  x <- qnorm(p, mean=mu, sd=sd)
  lines(c(x0, x,x), c(p,p,0), col=mgray)
  xmid <- (x0+x)/2
...
In TRIM, one of the basic assumptions is that observations, which are counts, are assumped to be Possion distributed, with probability mass function (PMF)
...
x <- 0 : (lambda*3)
y <- ppois(x, lambda)
plot(x, y, type='n', pch=16, xlab="Value", ylab="Cum. distr. func.", col=red) # empty
# background: indicate expected value
abline(v=lambda, col=mgray)
text(lambda, 0.1, expression(lambda), col=gray(0.5), pos=4)
# foreground on top
...
x <- 0 : (lambda*3)
y <- ppois(x, lambda)
plot(x, y, type='n', xlab="Value", ylab="Cum. distr. func.")
...
The approach used in TRIM is to enforce continuity by approximating the discrete Poison distribution by the continuous Gamma distribution, which probablity density function (PDF) and CDF are given by
$$ f(x; k, \theta) = \frac{1}{\Gamma(k)\theta^k} x^{k-1} e^{-x/\theta} $$
and
$$ F(x; k, \theta) = \frac{1}{\Gamma(k)} \gamma\left(k,\frac{x}{\theta}\right) $$
where $k$ and $\theta$ are a\emph{shape} and \empgh{scale} parameter.

The first two moments are given by
$$ \operatorname{E}[x]=k\theta  \quad\text{and}\quad  \operatorname{var}[x] = k \theta^2 $$

In order to fit the Gamma distribution on the Possion distribution, we equate the first two moments:
$$ k\theta=\lambda;\quad k\theta^2=\lambda \Longrightarrow k=\lambda;\quad \theta=1 $$
which suggest that a a Poisson distribution with rate $\lambda$ can be approximated by a Gamma distribution with $k=\lambda$ and $\theta=1$ for at least thge first two moments.
...
plot(xp, yp, type='s', pch=16, xlab="Value", ylab="Cum. distr. func.", col=red)
...
abline(v=lambda, lty="dashed", col=gray(0.8)) # mark expected value
text(lambda, 0.2, "Expected value", pos=4, col=gray(0.5))
...
The next step is to compute the multipliers, i.e. the ratio between quantiles corresponding to the 95%, say, confidence interval bounds and the standard errors. Unlike the normal distribution, where this ratio is a constant (1.96), for the gamma distribution the ratio will be dependend on the distribution parameter, i.e. expected value $\lambda$.
...
Here is an example how these multipliers can be used to compute confidence intervals for TRIM time totals, using the Skylark dataset, model 3 (independent year effects) and no overdispersion (i.e. Poisson-type variance assumptions).
```{r}
library(rtrim)
...
plot(tt)
lines(tt$time, tt$lo, lty="dashed")
lines(tt$time, tt$hi, lty="dashed")
```

In fact, the computation of confidence intervals is now built into RTRIM, using the `level' argument for `totals()'. The additional information is picked up by plot() automatically.
```{r}
...
This alternative constraint on variance is easily expressed using a negative multinomial distribution $\operatorname{NB}(r,p)$ where $r$ is a \emph{size} parameter and $p$ is a \emph{success probability}. In ecological applications al alternative parameterization is often used, using $r$ and mean $\mu$. In this case, the first two moments are given by
$$ \operatorname{E}[x] = \mu $$
and
...
plot(NULL, NULL, xlim=range(x), ylim=c(0,1), xlab="", ylab="CDF", type="n")
for (i in 1:n) {
...
Again, we use the methods of equal moments to find the Gamma parameters $k,\theta$ corresponding to the rexlaed assumptions $\operatorname{var}[x] = \sigma^2 \operatorname{E}[x] = \sigma^2\mu = \sigma^2\lambda$:
...
Mulitpliers relating standatd errors with confidence intervals bounds can be computed using the same approach as demonstrated above for the Poisson distribution. Here is an example for $\sigma^ = 1 \ldots 10$
...
Note that while upper-bound multipliers increase with overdispersion, lower-bound multipliers decrease. This is easily understood from the skewness of these distributions and the constrains that the lower bound is constrained to be $>0$. The following plot illustrates the principle for $\lambda=10$: and $\sigma^2=8$
```{r}
lambda <- 20
sig2   <- 5
alpha <- 0.05 # 95% CI
...
plot(x,y, xlab="", ylab="frequency", type='s', col=red)
...
sd <- sqrt(sig2 * lambda)
abline(v=c(lambda-sd,lambda+sd), lty="dashed", col=red)
...
abline(v=c(lo,hi), lty="dotted", col=red)
legend("topright", c("mean","std.err","C.I."), lty=c("solid","dashed","dotted"), col=red)
```

Things change when $\sigma^s > \lambda$, in this case the standard errors are larger than the mean, such that the multipliers must be $<1$ to ensure the confidence interval be positive. Here is an example for $\lambda=10$: and $\sigma^2=12$
```{r}
lambda <- 10
sig2   <- 12
alpha <- 0.05 # 95% CI
...
x <- 0 : 45
y <- dnbinom(x, mu=lambda, size=lambda/(sig2-1))
plot(x,y, xlab="", ylab="frequency", type='s', col=red, xlim=c(-5,45))
points(x, y, col=red, pch=16, cex=0.5)
...
It is recommended that these cases be avoided when fitting TRIM models.

The folowing diagram shows the multipliers for a range range of overdispersion parameters, indicating the domain of applicability (i.e. $\sigma^2 < \lambda$)
```{r}
lambda <- exp(seq(log(2.0), log(1000.0), len=100)) # uniform spacing in log-space
alpha <- 0.05 # 95% CI
...
```
Note that because of our feasibility constrains all mulitplier--exepected value relations look similar now, expect for the obvious shift to the right.
...
In this case, the effect of overdispersion is very limited. For an example where overdispersion is large, see the vignettes `Taming overdispersion' and `TRIM vs UIndex'.
