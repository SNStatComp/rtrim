...
For many species, count data is very skewly distributed. Let's look at an example for Oystercatcher data that comes with RTRIM.
...
You may recall from the Methods paper that the formula for overdispersion is given by
...
The question now is, how can be compute a more robust and realistic estimate of overdispersion, which may be (much) larger than 1, but not affected by artefatcs resulting from the estimation procedure.
...
plot(idx)
```
Where it should be noted that the solid band indicate standard errors and the dashed line indicates the 95% confindence interval.
