...
time <- dat$df$time
site <- dat$df$site
z <- trim_estimate(count,time,site, model=tcf$model,
...
W = wald(z)
print(W)
expect_equal(W$W,  as.numeric(target$wald$Wald.Test), tol=1e-2)
expect_equal(W$p,  as.numeric(target$wald$p), tol=1e-4)
